...
static Matrix *allocM(int r, int c) {
    ...
    if (!M) return NULL;
    M->r = r; M->c = c;
    M->a = (r * c == 0) ? NULL : calloc((size_t)r * c, sizeof(double));
    if (r * c != 0 && !M->a) { free(M); return NULL; }
    return M;
}
...
static Matrix *mulM(const Matrix *A, const Matrix *B, bool special_zero_inf) {
    if (!A || !B || A->c != B->r) return NULL;
    int nr = A->r, nc = B->c, nk = A->c;
    Matrix *R = allocM(nr, nc);
    if (!R) return NULL;
    for (int i = 0; i < nr; i++)
        for (int j = 0; j < nc; j++)
            for (int k = 0; k < nk; k++) {
                ...
                double y = B->a[k * nc + j];
                double term = x * y;
                if (special_zero_inf && isnan(term) &&
                    ((isinf(x) && y == 0.0) || (isinf(y) && x == 0.0))) {
                    term = INFINITY;
                }
                R->a[i * nc + j] += term;
            }

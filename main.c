...
static double det_matrix(const Matrix *A) {
    ...
    for (int i = 0; i < n; i++) {
        int piv = i;
        double mx = fabs(M->a[i*n+i]);
        for (int r = i+1; r < n; r++) {
            double v = fabs(M->a[r*n+i]);
            if (v > mx) { mx = v; piv = r; }
        }
        if (mx < 1e-15) { det = 0; break; }
        if (piv != i) {
            for (int j = 0; j < n; j++) {
                double t = M->a[i*n+j];
...
int main(int argc, char **argv) {
    ...
    const char *in1 = NULL, *in2 = NULL, *out = NULL;
    if (!strcmp(op, "sum") || !strcmp(op, "sub") || !strcmp(op, "mul") ||
        !strcmp(op, "pow") || !strcmp(op, "det")) {
        if (argc == 5) { in1 = argv[2]; in2 = argv[3]; out = argv[4]; }
        else if (argc == 4) { in1 = argv[2]; out = argv[3]; }
